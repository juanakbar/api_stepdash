...
class MekanikController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $customerRole = Role::where('name', 'Mekanik')->first();
            $user->assignRole($customerRole);
            flash()->success('Mekanik Dengan Username ' . $request->username . ' Berhasil Ditambahkan');
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            flash()->success('Mekanik Dengan Username ' . $request->username . ' Berhasil Diperbarui');
            return redirect()->back();
        } catch (\Exception $e) {
            // Menangani kesalahan dengan mengirimkan pesan error
            flash()->error('Terjadi kesalahan saat memperbarui Mekanik: ' . $e->getMessage());
    ...
    public function destroy($id)
    {
        ...
        $user->roles()->detach();
        $user->delete();
        flash()->success('Mekanik Dengan Username ' . $user->username . ' Berhasil Dihapus');
        return redirect()->back();
    }
}
